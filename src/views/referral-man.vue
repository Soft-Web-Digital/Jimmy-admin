...
const { getReferralsManagementData, editSystemData } = useNetworksStore();
const { referrals_management, loading, dialog, dialog2, content } = storeToRefs(
  useNetworksStore()
);

onMounted(async () => {
  await getReferralsManagementData();
});

const page_title = ref({ title: "Referral Management" });
const title = ref("");
const header = ref([
  {
    title: "Id",
      ...
      <v-card class="pa-5">
        <v-table>
          ...
          <tbody v-if="loading == false">
            <tr class="pa-3" v-for="(data, index) in referrals_management" :key="data?.id">
              <td>{{ index + 1 }}</td>
              <td>{{ data?.content }}</td>
              <td>{{ data?.hint }}</td>
              <td>{{ data?.title }}</td>
