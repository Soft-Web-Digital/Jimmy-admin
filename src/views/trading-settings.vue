...
const { getTradingData, editSystemData } = useNetworksStore();
const { tradings, loading, dialog, dialog2, content } = storeToRefs(
  useNetworksStore()
);
...
const page_title = ref({ title: "Trading Settings" });
const title = ref("");
const header = ref([
  {
    title: "Id",
      ...
      <v-card class="pa-5">
        <v-table>
          ...
          <tbody v-if="loading == false">
            <tr class="pa-3" v-for="(data, index) in tradings" :key="data?.id">
              <td>{{ index + 1 }}</td>
              <td>{{ data?.content }}</td>
              <td>{{ data?.hint }}</td>
              <td>{{ data?.title }}</td>
